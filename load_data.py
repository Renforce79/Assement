...
from utils.db_connect import connections
from utils.test import test_dataframe
import os
dag_folder = os.path.dirname(os.path.abspath(__file__))

# Establish a connection to the PostgreSQL database
connect = connections()
conn = connect.posgres_conn()
cursor = conn.cursor()
...
# Remove the last 12509 or more rows because have comments
df = df.iloc[:-2]

# Save the filtered dataframe to a new CSV file
df.to_csv('{}/filtered_BING_MultiDays.csv'.format(dag_folder), index=False)
n = df.shape[1]
td = test_dataframe(df,n)
try:
    tdf = td.test()
